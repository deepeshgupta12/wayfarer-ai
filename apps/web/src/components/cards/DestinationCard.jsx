...
import { Star, Heart, Sparkles, Image as ImageIcon } from 'lucide-react';
    ...
    return (
      ...
      >
        <div className="relative h-32 overflow-hidden bg-secondary">
          {primaryImage ? (
            <img src={primaryImage} alt={name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          ) : (
  ...
  return (
    ...
    >
      <div className="relative h-52 overflow-hidden bg-secondary">
        {primaryImage ? (
          <img src={primaryImage} alt={name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-muted-foreground">
        ...
        {tags.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            {tags.slice(0, 4).map((tag) => (
              <span key={tag} className="px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground text-xs capitalize">
